...
float cloudDf(vec3 pos, float rain) {
    #if NL_NOISE_TYPE == 1
    //pos.xz += 0.4*noise(3.0*pos.xyz);
	pos.xz += 0.2*noise(6.0*pos.xyz);
	//pos.xz+= 0.2*rand(0.2*pos.xz);
    #elif NL_NOISE_TYPE == 2
    pos.xz += 1.0*noise(1.5*pos.xyz);
	pos.xz += 0.2*noise(6.0*pos.xyz);
	pos.xz+= 0.2*rand(0.2*pos.xz);
    #endif
  ...
  // rain transition
  vec2 t = vec2(0.2501+0.2*rain, 0.25+0.2*rain*rain);
